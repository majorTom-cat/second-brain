...
## 위임 프롬프트 주입 세트 (2026-07-06, chaebo 교훈 — [[inject-lessons-into-delegation]])

서브에이전트는 auto-memory·CLAUDE.md·브리프를 **읽지 않는다** — 위임 프롬프트가 그의 전체 세계다.
bulk 위임 프롬프트엔 최소 다음을 **명시 주입**한다(누락 시 위임 산출물이 프로젝트 원칙을 어긴다 —
실증: chaebo S/M 약어[쉬운 말 위반]·손그림 악보[도메인 표준 위반], 둘 다 위임 산출물):
1. 사용자/대상 특성 (예: 비전문가 — 쉬운 말, 전문 약어 금지)
2. 도메인 표준·대조 기준 (예: 악보=실제 조판과 대조)
3. 프로젝트 경계 (라이선스 제약·비목표·보안 경계)
4. 검증 기준 ("완료 보고 전 렌더/실행 관찰" 등)
대상 프로젝트 CLAUDE.md 에 "주입 세트" 목록을 박아두면 세션마다 재작성하지 않는다(chaebo CLAUDE.md §2 방식).
