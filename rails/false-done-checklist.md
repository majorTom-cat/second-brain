...
- [ ] **도메인 표준 산출물의 손그림 근사** — 악보·지도·차트·수식처럼 *조판/표기 표준이 있는* 산출물을 SVG/CSS 로 손수 흉내내면 "렌더 됨·JS 에러 0" QA 는 전부 통과하는데 전문가 눈엔 즉시 비표준(겹침·간격·기호 오류). 시안이라도 제품이 쓸 **실제 렌더러를 내장**해 그리는 게 근본해결(+라이브러리 검증 선행 효과). 관찰: 실제 렌더러 출력 또는 실제 제품 스크린샷과 *나란히* 대조 — "그려졌다"로 통과 금지. 🔬(chaebo 2026-07-06: 손그림 타브 2회 반려 → alphaTab 내장으로 해소) [[use-real-renderer-not-imitation]]
## C. 권한/불변식
...
- [ ] **위임 산출물의 프로젝트 원칙 위반(주입 누락)** — 브리프·CLAUDE.md 의 원칙(쉬운 말·도메인 표준·라이선스 경계)이 위임 프롬프트에 안 실리면 서브에이전트에겐 *존재하지 않는* 규칙 — 산출물은 그럴듯한데 원칙을 어긴 채 done 보고(H 의 R4a 렌즈 구체화). 관찰: 위임 전 주입 세트(routing.md §주입 세트) 점검 + 산출물을 원칙 목록과 대조. 🔬(chaebo 2026-07-06: S/M 약어·손그림 악보 — 둘 다 위임 산출물) [[inject-lessons-into-delegation]]
