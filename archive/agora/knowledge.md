...
기획팀이 프로토타이핑을 거쳐 **PRD v4.1**(요구사항+화면+디자인 통합본)을 확정해 들어온 두 번째 인테이크 라운드. v3.0을 풀 구현해 운영 중인 코드에 **델타만** 얹는 작업(=새로 만들지 않고 수정이 빠르다가 실제로 증명됨). 운영 `main` 격리(`redesign/v4.1` 브랜치)에서 진행, 자동검증까지 완료(최종 177 테스트 green)·디자인 이미지 스킨+`dev→main` 머지만 대기. (`E:\agora\docs\changes\CR-001-prd-v4.1.md`)
...
- **프로토타입 골격 vs 기존 화면 혼동**: 새 프로토타입을 받아 "수정이 빠르다"며 진행했더니, 산출 화면이 **기존 v3.0 레이아웃과 거의 동일**해 사용자가 "새로 다시 할까" 회의. 원인 = 백엔드/데이터 델타에 집중하느라 **셸·IA·내비 골격을 프로토타입에 맞추지 않음**. 교훈: 비주얼은 토큰으로 나중에 흡수하더라도 **셸+홈→로그인→캘린더 골격(상단 GNB·레이아웃·정보구조)은 프로토타입에 맞춰 먼저 가시화**하고 사용자 방향 확인을 받을 것. 프로토타입 HTML의 화면 앵커(`w-*`)를 마커로 렌더 정합을 스모크 검증.
