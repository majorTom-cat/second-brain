...
2. **경로 해석**: `rails/project-paths.md` 규약으로 `<slug>` → `$DOCS`/`$META`/`$STATE`/`$CODE` 를 확정한다
   (미등록 slug 는 internal = `projects/<slug>/` 기존 동작). 이하 모든 산출/입력 경로는 이 변수로 쓴다.
3. `$STATE`(pipeline.yaml)가 이미 있으면 **재실행 모드**: 기존 산출물과 창작 `$META/HANDOFF.md` 를 읽고
   부족/지적된 부분만 보강한다(처음부터 다시 쓰지 않는다).
4. 없으면 창작 `$META`·`$DOCS` 디렉터리를 만들고 `rails/artifact-templates/creative/*` 를 복사해 시작점으로 삼는다.
...
목표/제약 기준으로 **심사·병합**`[tier: judgment]` → 승자 기준으로 나머지 문서를 `$DOCS/` 에 합성:
