...
## 2026-06-28~29 — 캘린더 색 인코딩 라운드(부서↔자원 구분) · 실패→교훈

**맥락**: 사용자가 "차량색↔부서색 구분 안 됨" 지적. 막대 색을 부서→자원으로 바꾸고(의도적 시안 divergence, ADR `intra/docs/calendar-color-decision.md`), 색 구분을 여러 라운드 손봄. **사용자가 *세 번* 되돌려보냄** = 내 검증이 표면에 머문 신호.

- **★위치 이동 ≠ 해결.** 1차 시도가 *막대 면=부서/점=자원* 을 *면=자원/점=부서* 로 **뒤집기만** 하고 두 팔레트는 *같은 hue 바퀴*로 둠 → 충돌 그대로. 근본은 "두 색 세트를 **독립 hue**(따뜻 vs 차가운)로 분리"였고 **시안 `spec-calendar-color.html` 이 '독립 토큰으로' 라고 *명시*했는데** 내가 덜 따랐다. 증상을 옮기면 진행한 듯하나 사용자는 또 겪음. 정본 [[solve-root-not-relocate-measure-real-metric]].
- **★프록시 지표 ≠ 진짜 지표.** "hex 다름(∩=∅)" "톤으로도 구분" 을 구분 근거로 댔다 = 구조적 프록시. 실제론 **ΔE(CIEDE2000) 실측 + 렌더 눈확인**이라야. hex 다른 두 색이 ΔE 8.9(거의 같음)였다. 측정하고서야 warm/cool 분리(교차 ΔE 35)·차량끼리 보강(18.5→28.3)·확장 16색(교차 21.2)이 *증명*됐다. "검색해서 찾아보라니까 못찾았잖아"(사용자) = 리서치·측정을 늦게 함.
- **★로컬 편의는 영구화.** 관리자 로그인을 `hong` 별칭으로 만들었는데 테스트 `resetDb()`→`db:reset` 재시드가 매번 시드값 `hng7k2qz9a` 로 되돌려 hong 이 사라짐 → 사용자 **2번** "안 된다". 고침=`db:reset`(=reseed+`setpw.local` hong) / `db:reseed`(raw, 테스트·inspect용)로 분리해 충돌 없이 영구화. "내가 방금 됨" ≠ "사용자가 언제와도 됨". 정본 [[make-local-conveniences-durable]].
- **확장성은 처음부터 물어라.** 팔레트 10색=하드 10-cap 이었는데 사용자가 "여러 개 추가 다 고려했나" 물어서야 16색+progressive disclosure(10 노출, 소진 시 공개)로 확장. 색은 본질적으로 5~7개까지 또렷(리서치) → 그 이상은 이름표가 주 식별(설계 한계로 ADR 에 명기). 부서 create 서버가드도 차량/회의실과 *불일치*하던 걸 이때 발견·보완.
- **잘된 것(성공패턴)**: 매 변경 ΔE 실측 스크립트(scratchpad `finalize`·`verify-split`·`scale`·`extend`.mjs) + 데스크톱·모바일 렌더 + 전 변경 ADR 누적 + 충실도 배터리가 *안 잡는* divergence 라 ADR 이 유일 추적장치임을 실측 확인([[intentional-design-divergence-needs-adr]]).
- **반응형 균일 줄바꿈(차량 일간 헤더 주차배지)**: 좁은 화면서 헤더가 본문 컬럼과 어긋나고 배지가 차량마다 제각각 내려감. ① `repeat(N,1fr)`=`minmax(auto,1fr)`라 내용 적은 컬럼만 축소 → `minmax(0,1fr)`로 동일폭. ② flex-wrap=내용 기준 → 배지 긴 차량부터 스태거 → `container-type:inline-size`+`@container(max-width:T)`로 셀 폭 기준 *일제히*. T 는 content-box(padding 빼고) 기준이라 측정으로 정함(겉보기 250→실제 content-box 237). 두 번에 나눠 고침(flex-wrap 부분→minmax+container query 완성) = *한 번에 근본*까지 안 가서 사용자 재지적. 정본 [[responsive-uniform-wrap-container-query-not-flexwrap]].
