...
## ★추가 교훈 — 운영 디버깅 세션 (2026-06-19, deploy-only 후속)

배포 후 *운영 단계*에서 나온 교훈. 앱 팀이 기능을 추가하면 **배포 담당이 새 런타임 설정을 이어 맞춰야** 한다 — deploy-only 의 핵심 인터페이스.

- **앱 팀의 보안/기능 추가 = 운영자에게 설정 핸드오프.** 코드 오너가 (서버측) JWT 인증을 붙이자, 앱 소스는 안 건드려도 **운영자가 새 런타임 설정을 클러스터에 주입**해야 동작: 서명키(k8s Secret, 없으면 *기동 거부* 안전장치)·허용 오리진(env). ★함정: *그 설정이 들어가기 전엔 새 이미지가 못 떠* 옛 이미지가 계속 돌며 같은 증상 → **"고쳤는데 그대로"**. **언제 같게**: 앱 팀과 운영이 분리된 모든 배포(시크릿/env 핸드오프를 인계 목록으로 명문화).
- **런타임 시크릿은 CI 변수가 아니라 k8s Secret.** CI 변수는 빌드/배포 잡 동안만 존재 → 파드는 못 봄(러너 SA 도 보통 시크릿 생성 권한 없음). 시크릿은 `secretKeyRef`, 일반설정은 매니페스트 env.
- **CORS 403 은 인증 에러로 위장한다.** same-origin SPA(프록시)라도 **TLS 종단 뒤면** 앱이 교차출처로 보고, 허용 오리진 미설정 시 403 — 브라우저 CORS 에러가 아니라 *서버 403*. 운영 호스트를 허용 오리진에 명시.
- **배포 산출물 ≠ 소스 를 의심 + 로그 한 줄로 판정 금지.** 진범이 *소스엔 없는데 이미지엔 있던* 보안 기본설정인 사례. 그럴듯한 기동 로그 한 줄은 지표가 아닐 수 있다(커스텀 보안설정에서도 기본 메시지가 뜸). 진단은 **행동(엔드포인트 직접 호출 상태코드)·실제 예외 스택**으로([[consult-prior-art-first]]).
- **at-rest 암호화 키는 영구 불변 = 운영 1급 제약.** 키 변경/데이터-키 불일치 → 복호화 `Tag mismatch` → 조회 500, 옛 키 없으면 **복원 불가**(GCM). 데이터 이관·수동 INSERT 때 키 연속성 필수. (위험 2 가 현실화된 사례.)

## 레일 시사 추가 (제안 — 사람 승인)
- `patterns/intranet-deploy.md` 에 "운영 디버깅 함정"(시크릿=Secret≠CI변수 · CORS-behind-TLS 403 · 이미지≠소스/로그한줄금지 · at-rest키 영구성) 일반화 보강 후보.
- `deploy-runbook` intranet 체크리스트: "앱 팀의 보안/기능 추가에 따라오는 운영자 설정(시크릿·env) 핸드오프 확인" 항목 후보.
