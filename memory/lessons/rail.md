...
### 열린 결정 — 승인 채널: 하드 분리(A) vs 탐지(P-U4)  (2026-06-20 사용자와 합의)
- **결정**: 1인·감독 운영에선 **P-U4(탐지+규약)로 충분 — 하드 분리(A)는 걸지 않는다.** (A=Claude Code 권한으로 승인 파일 쓰기를 Claude에게 금지 → 사람만 도장.)
- **트레이드오프**: (A)는 `approved=진짜 사람`을 *구조로* 보장하지만, 핸드오프마다 사람이 직접 도장 → "승인하고 바로 다음" 식 자율 흐름에 마찰. 그런데 도장의 보증은 **사람이 게이트에 *있다*는 사실**이 이미 제공한다(1인 감독 운영에선 벽이 같은 안전을 두 번 사는 셈, dim5 단순성).
- **언제 (A)로 바꾸나(되돌릴 신호)**: 사람이 게이트에 *안 있을 수 있는* 운영으로 갈 때 — **무인 자동(cron/풀오토)·다인·핸드오프**. 예: intra 를 무인 배포 cron 으로 돌리게 되면 그때 (A) 재검토.
- **언제 이대로(P-U4)**: 1인 + 항상 검토(=현재). → 다음 세션이 "메타 한계!" 보고 (A)를 또 제안하지 않게 — *안 한 게 아니라 맥락에 맞춰 안 한 것*.
