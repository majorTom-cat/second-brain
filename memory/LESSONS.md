...
- [intra (2026-06-22 배포 LIVE)](lessons/intra.md) — 사내 k8s 실배포(`<앱도메인>`). 전이교훈: **standalone 이미지 함정은 "dev통과/이미지에서만 터짐"**(npm lock·@/db import throw·sharp·dotenv 4개) → **배포 전 로컬 `docker build`+`run` 스모크**가 정답(`npm run build`론 못 잡음) · **capture≠apply 재발**(산문 교훈 자동로드 안 돼 또 잊음 + port-forward 2번 거꾸로 안내) → 함정은 *복사템플릿+실행체크*로 · **사내 Node앱 인프라**(DB포트 프로젝트별·시크릿 서버직접·엔트리포인트 migrate·CI SA=set image만) · **port-forward는 서버tmux=로컬재부팅 무관** · **prod시드≠dev데모시드**. → 승격: [patterns/bns-nextjs-deploy-starter.md](patterns/bns-nextjs-deploy-starter.md) + [[standalone-image-preflight-smoke-test]]; false-done "이미지 미실행 done 금지" 추가.
- [chaebo](lessons/chaebo.md) (2026-07-06) — **창작 단계 완주(REQ 47·시안 6화면·조사 5종·라이선스 감사)**: 발산 3안이 "단계"로 수렴(로드맵 합성) · 배포 결정 즉시 라이선스 감사 · 합성 픽스처 acceptance · 스파이크 실패=강등 게이트 명문화. 실패: 시안 벤치마크 생략(v1)·도메인 표준 손그림 2회(v3→alphaTab 근본해결)·위임 주입 누락(S/M)·동시작업 add -A → 반영: ✅R1(/creative §2.5 시안 하드규칙)·R2(routing 주입 세트)·R3(false-done A·G 2건)·auto-memory 2건·chaebo CLAUDE.md 이식.
